...
public class BlankInputLessorBankDetails extends BaseClass {
    //Test Case ID in Excel= Lessor_REG015
////Bank Details Field Verify on the Blank Input
    @Test(dataProvider = "getData", dataProviderClass = TestData.class)
    public void blankInputBankDetails(HashMap<String, String> input, ITestContext context) throws InterruptedException, ParseException {
        boolean e_InvState;
        String msme_Option;
        ...
        soft_Assert.assertEquals(lessorPageObj.getToastMessage("Data save successfully").get(0), "Data save successfully", "Address Toast Message Value are not Equal"); //Assertion For Successfull Address save verifying from the toast message
        lessorPageObj.clickNextButton();
        ...
        lessorPageObj.clickSaveButton(1);
        soft_Assert.assertEquals(lessorPageObj.getToastMessage("Data save successfully").get(0), "Data save successfully", "Contact Person Toast Message Value are not Equal");
        lessorPageObj.clickNextButton();
        ...
        //  if (msme_Option.equals(" Medium ") || msme_Option.equals(" Small ") || msme_Option.equals(" Micro ")) {
        ...
        lessorPageObj.clickSaveButton(1);
        soft_Assert.assertEquals(lessorPageObj.getToastMessage("Data save successfully").get(0), "Data save successfully", "Compliance Toast Message Value are not Equal");
        ...
        soft_Assert.assertEquals(lessorPageObj.getToastMessage("Data save successfully").get(0), "Data save successfully", "GST Details Toast Message Value are not Equal");
        lessorPageObj.clickNextButton();
        ...
        lessorPageObj.clickAddButton();
        lessorPageObj.clickSaveButton(1);
        String[] expected_Msgs = {"Bank Name Required", "Account Holder Name Required"};
        List<String> msgs = lessorPageObj.getBankFieldErrMsg();
        for (String actual_Msg : msgs) {
            soft_Assert.assertEquals(actual_Msg, expected_Msgs[msgs.indexOf(actual_Msg)], "Bank Details Field Error Msgs Values are not equal");
        ...
        soft_Assert.assertTrue(lessorPageObj.getBankFieldErrMsg().isEmpty());
        lessorPageObj.clickSaveButton(1);
