...
public class InvalidInputLessorComplianceTab extends BaseClass {

    String msme_Option;
    //Test Case ID in Excel= Lessor-REG012

//Putting Invalid data and uploading invalid files into complinace fields
//Error Messages Validation on Saving Compliance Datails on Blank Field.

    @Test(dataProvider = "getData", dataProviderClass = TestData.class)
    public void invalidDataCompliance(HashMap<String, String> input, ITestContext context) throws InterruptedException, ParseException {
        ...
        lessorPageObj.setPanNo(input.get("pan_Number") + "*(SJASJHDK");
        soft_Assert.assertEquals(lessorPageObj.getPanErrorMsg(), "PAN Number not valid", "Pan Number Field Error Message Values are not Equal");
        lessorPageObj.clearPanNo();
        ...
        if (!msme_Option.equals(" Not Available ")) {
            ...
            soft_Assert.assertEquals(lessorPageObj.getComplianceErrorMsg().get(0), "MSME Registration Number minlength_12", "MSME Field Error Message Values are not Equal");

            lessorPageObj.clearMsmeRegNo();
            lessorPageObj.setMsmeRegNo(input.get("msme_Registration_Number") + "123344");
