...
public class BlankInputSubCodeMap extends BaseClass {

    //Test Case ID in Excel= Lessor_REG-019
    @Test(dataProvider = "getData", dataProviderClass = TestData.class)
    public void blankInputLessorSubCodeMap(HashMap<String, String> input, ITestContext context) throws InterruptedException, ParseException {
        boolean e_InvState;
        String msme_Option;
        ...
        soft_Assert.assertEquals(lessorPageObj.getToastMessage("Data save successfully").get(0), "Data save successfully", "Address Details Toast Message Value are not Mathching"); //Assertion For Successfull Address save verifying from the toast message
        lessorPageObj.clickNextButton();
        ...
        lessorPageObj.clickSaveButton(1);
        soft_Assert.assertEquals(lessorPageObj.getToastMessage("Data save successfully").get(0), "Data save successfully", "Contact Person Toast Message Value are not Mathching");
        lessorPageObj.clickNextButton();
        ...
        lessorPageObj.clickSaveButton(1);
        soft_Assert.assertEquals(lessorPageObj.getToastMessage("Data save successfully").get(0), "Data save successfully", "Compliance Toast Message Value are not Mathching");
        ...
        soft_Assert.assertEquals(lessorPageObj.getToastMessage("Data save successfully").get(0), "Data save successfully", "GST Details Toast Message Value are not Mathching");
        lessorPageObj.clickNextButton();
        ...
        soft_Assert.assertEquals(lessorPageObj.getToastMessage("Opps something went wrong.\n" +
                "site_code : This field may not be null.").get(0), "Opps something went wrong.\n" +
                "site_code : This field may not be null.", "SubCode Toast Message Value are not Matching");


        lessorPageObj.clickSaveButton(1);
        String[] expected_Msg = {"Site Address Required", "Lessor Site Name Required", "Lessor Site Code/ID Required"};

        List<String> err_List = lessorPageObj.getSubMapErrMsg();
        for (String actual_Err : err_List) {
            soft_Assert.assertEquals(actual_Err, expected_Msg[err_List.indexOf(actual_Err)]);
